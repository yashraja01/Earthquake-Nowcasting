...
for city, (city_lat, city_lon) in cities.items():
    ...
    #Check if distance is within 150 km and Mw>6, returns true or false
    within_radius_and_magnitude = (distance_km <= 150) & (df['Mw'] > 6)
    ...
    df[city_distance_col] = np.where(within_radius_and_magnitude, distance_km.round(2), np.nan)
    df[city_date_col] = np.where(within_radius_and_magnitude, df['Date'], np.nan)
    df[city_mag_col] = np.where(within_radius_and_magnitude, df['Mw'], np.nan)
